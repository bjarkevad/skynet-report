...
The proposed planning procedure for a multi-agent system can be divided in to "??????? four ????????" algorithms. The first being the distribution of goals between agents, done with a bidding algorithm\pcite{VanderKrogt2005}. 
...
\subsubsection{Heuristics}
 % with modifications to fit specific cases.
...
\todo[inline]{Maybe change below line to something about subgoals and subgoal heuristics} 
